...
class WatchMenController extends Controller
{
    public function index()
    {
        ...
        $products = Product::where("type_id", 1)->where("quantity", ">", "0")->get();
        $suppliers = Supplier::all();
        $blogall = New_new::all();
        // $products = $this->product->latest()->take(6)->get();
        return view('client.type.men', compact('products', 'suppliers', 'blogall'));
    }

    public function filter(Request $request)
    {
        $products = Product::where("type_id", 1)->where("quantity", ">", "0");
        $blogall = New_new::all();
        $suppliers = Supplier::all();
